...
__author__ = "Jianke Wu"
__copyright__ = "Copyright 2018, The Anti-Plugin Project"
__version__ = "1.0.0"
__email__ = "wujianke@corp.netease.com"
__phone__ = "18868100925"
__description__ = "离线训练模块 - 特征工程类，" \
...
class DataLoader(object):
    ...
    def __init__(self, source_path, logid_path, label_tags, test_size, sampling_type):
            ...
            for j, filename in enumerate(os.listdir(path)):
                ...
                try:
                    with open(filepath, 'rb') as f:
                        ...
                        self._data_ids.append(filepath.split('/')[-1].split('.')[0])
    ...
    def _sampling(self):
        ...
        if self._sampling_type == 'up':
            sampling_idx = range(len(self._label_data))
            ...
            for i, label_index in enumerate(labels_index):
                sampling_idx.extend([label_index[random.randint(0, len(label_index)-1)]
                                     for _ in xrange(max_labels_index_len-len(label_index))])
        else:
            sampling_idx = list()
    ...
    def run_load_only(self, split=True):
        self._load()
        if split:
            self._split()
...
class EvseqLoader(DataLoader):
    def __init__(self, source_path, logid_path, label_tags=list(['normal', 'waigua_not_merge']), test_size=0.2, sampling_type='up'):
        DataLoader.__init__(self, source_path=source_path, logid_path=logid_path, label_tags=label_tags, test_size=test_size,
                            sampling_type=sampling_type)
