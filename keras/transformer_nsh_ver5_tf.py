...
if __name__ == '__main__':
    ...
    config = tf.ConfigProto(intra_op_parallelism_threads=hp.intra_op_parallelism_threads,inter_op_parallelism_threads=hp.inter_op_parallelism_threads,device_count = {'CPU' :1, 'GPU' : 0})
    ...
    embeddings = Embedding(max_features, 16)(S_inputs_x)
    embeddings = Position_Embedding()(embeddings) # 增加Position_Embedding能轻微提高准确率

    O_seq = Attention(8,4,S_inputs_time)([embeddings,embeddings,embeddings])
    O_seq = Flatten()(O_seq)
    ...
    y_train = np.array([[1,0] if x==1 else [0,1] for x in y_train_list]) if OUTPUT_UNIT==2 else np_utils.to_categorical(y_train_list,OUTPUT_UNIT)
    y_test = np.array([[1,0] if x==1 else [0,1] for x in y_test_list]) if OUTPUT_UNIT==2 else np_utils.to_categorical(y_test_list,OUTPUT_UNIT)
